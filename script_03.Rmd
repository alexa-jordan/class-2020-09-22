...
Wisdom: Define precisely what we are looking for, or what we want to know. In this case we want to find the difference between the new and old course enrollment numbers. 


After we have a model, Temperance suggests we should:

Temperance: This data may not fully reflect the course enrollment. For example, we might not be looking at the same data from the same individuals. We need to recognize the limits of the model. 
...
the_data <- inner_join(sep_old, sep_new, 
                       by = c("id", "title", "name", "department")) %>% 
  mutate(differ = u_grad.y - u_grad.x)  
  
big_dec <- the_data %>% 
  filter(str_detect(differ, "-") == TRUE) %>% 
  arrange(differ) %>% 
  slice(1:5)

big_inc <- the_data %>% 
  filter(str_detect(differ, "-") == FALSE) %>% 
  arrange(desc(differ)) %>% 
  slice(1:5)

plotting_data <- full_join(big_inc, big_dec, 
                           by = c("id", "title", "name", "department",
                                       "u_grad.x", "u_grad.y", "differ")) 
ggplot(plotting_data, mapping = aes(reorder(name, differ), differ)) + 
  geom_col(fill = "blue") +
  theme(axis.text.x=element_text(angle = 85, vjust = 0.5)) +
  labs( 
    title = "Change in Course Enrollment \n Fall 2020: September 1-22",
    x = "Course Title", 
